<template>
  <div ref="page">
    <div class="app-container" v-if="dataList.name">
      <div class="user-list" style="margin-top: 0;">
        ...
        <div class="detailed-bor" style="position: relative;">
          <div class="detailed-bor-info">
            <el-select v-model="percent" placeholder="请选择" @change="statusChange" style="width:100px" size="small">
...
<script>
/* eslint-disable */
import { getCompanysQcc } from '@/api/companys'
...
export default {
  ...
  created() {
    ...
    _this.getCompanysQccBox(this.$route.query.id)
  },
  watch: {
  },
  methods: {
    getCompanysQccBox(id){
      var _this = this
      var page = {
      ...
      getCompanysQcc(id, page).then(response => {
        loading.close();
        if (response.code === 'OK') {
    ...
    statusChange() {
      var _this = this
      _this.stocklist = {}
      _this.getCompanysQccBox(_this.id)
    },
    goXlsx() {
